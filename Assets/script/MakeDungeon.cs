...
public class MakeDungeon : MonoBehaviour
{
    ...
    public void Start ()
    {
        ...
        var startObj = Instantiate(prayer, new Vector3(startPos.x, 0, startPos.y), Quaternion.identity);
        var goalObj = Instantiate(goal, new Vector3(goalPos.x, 0, goalPos.y), Quaternion.identity);
        // var prayerObj = Instantiate(prayer, new Vector3(startPos.x, 0, startPos.y), Quaternion.identity);
    ...
    // マップ生成
    public Vector2Int MakeMapInfo ( Vector2Int _startPos )
    {
        //スタート位置配列を複製
